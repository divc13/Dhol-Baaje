...
export class LucidResolver {
  ...
  async mintNft(
    ...
    const cbor = await this.transactionService.getSignedNftMintingCbor(
      seedPhrase,
      nft
    );
  ...
  async mintAssets(
    ...
    const seedPhrase = req.headers[this.SEED_PHRASE_HEADER];
    const cbor = await this.transactionService.getSignedAssesMintingCbor(
      seedPhrase,
      asset
    );
  ...
  async transfer(
    ...
    const seedPhrase = req.headers[this.SEED_PHRASE_HEADER];
    const cbor = await this.transactionService.getSignedTransferCbor(
      seedPhrase,
      transfer
    );
