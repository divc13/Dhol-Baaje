...
import { LikedTracksFlureeMap } from '../../mappers/fluree';
import { ILikedTracksRepository } from '../interfaces';

@Injectable()
export class LikedTracksFlureeRepository implements ILikedTracksRepository {
  constructor(
    private fluree: FlureeClient,
    private mapper: LikedTracksFlureeMap
  ) {}

  public async exists(id: string): Promise<boolean> {
    const query = selectOne('likedTracks/identifier')
      .where(`likedTracks/identifier = '${id}'`)
      .build();
    const fql = compile(query);
    const result = await this.fluree.query(fql);
    return !!result && result['likedTracks/identifier'] === id;
  }
  ...
  public async findAll(selectionSetList?: string[]): Promise<LikedTracks[]> {
  ...
  public async findManyById(
    ...
    let spec;
    if (idList.length > 0) {
      spec = select('*').where(`likedTracks/identifier = '${idList[0]}'`);
      if (idList.length > 1) {
        idList.shift();
        idList.map((id: string) => {
          spec = spec.or(`likedTracks/identifier = '${id}'`);
        });
      }
    }
  ...
  public async save(
    ...
    let spec;
    const data = this.mapper.fromEntity(likedTracks);
    const existing = await this.findOneById(likedTracks.id, selectionSetList);
    if (existing) {
      ...
      processUpdateTransactSpec(
        update('likedTracks').data(resolvedData).build(),
        updateTransact,
        createTransact
      );
      ...
      const existingSpec = [];
      processUpdateTransactSpec(
        update('likedTracks').data(existingData).build(),
        existingSpec
      );
      spec = reconcileArrays(updateTransact, existingSpec);
    } else {
      data._id = `likedTracks$${likedTracks.id}`;
      spec = create('likedTracks').data(data).build();
    }
    ...
    if (response.status === 200) {
      return await this.findOneById(likedTracks.id, selectionSetList);
    } else {
