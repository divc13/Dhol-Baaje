...
interface BuySellParams {
    action: string;
    track: Track;
    playlist: Track[];
  }

function BuySell({action, track, playlist} : BuySellParams) {
  ...
  return (
    <div className="flex-col items-center px-2 py-1">
      <div className="flex w-full gap-3 items-center py-2">
        ...
        <div className="flex-grow flex flex-col">
          ...
          <p className="truncate text-xs text-[#686868] font-semibold hover:underline">
            {(track?.owner?.email || "").replace("-", " ").slice(0, 15) || ""}
          </p>
        </div>
      ...
      <button className="flex mx-7 px-5 hover:scale-110 bg-green-700 rounded py-1 border border-[#15883e]">
            {action}
      </button>
